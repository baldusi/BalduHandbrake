...
6. Grab the latest release from the [Official Repository](https://github.com/baldusi/BalduHandbrake/releases) and unpack it in the directory you want to use. Unpack the BladuHandbrake directory, not only it's contents, as Arduino IDE requires the .ino and directory names to match.

## 3. Configure and Flash (3 min)

1. Start up Arduino IDE and open `BalduHandbrake.ino`.
2. Edit `config.h`:
   - Choose your ADC (`#define SENSOR_ADC_xxx`)
...
4. Verify → Upload.
5. Upload might fail as the ESP32 enter into bootmode. Just select the new COM port and hit Upload again.
